...
from utils import json_load, ICLDataset
...
from utils import parse_label, ICL_PROMPT_DICT, prompt_template
...
class DataTrainingArguments:
    ...
    max_new_tokens: int = field(
        default=4, metadata={"help": "the max number "}
    )
...
def main():
    ...
    # first init the in-context learning examples
    train_dataset = raw_datasets["train"]
    data_type = data_args.train_data_path.split("/")[-2].split("_")[0]
    if is_main_process(local_rank):
        logger.info(f"data type: {data_type}, ICL: 10 shot")
    assert data_type in {"noun", "verb", "event", "merged"}

    alpaca_prompt, alpaca_ins_prompt = ICL_PROMPT_DICT["prompt_input"], ICL_PROMPT_DICT["prompt_input_ins"]
    cur_template = prompt_template[data_type]
    instruction = cur_template["instruction"]
    cur_exemplar_list = train_dataset[:10]
    new_exemplar_list = [alpaca_ins_prompt.format(instruction)]
    for idx, d in enumerate(cur_exemplar_list):
        input_text, output_text = d["input"], d["output"]
        alpaca_text = alpaca_prompt.format(input_text) + " " + output_text + "\n\n"
        new_exemplar_list.append(alpaca_text)
    exemplar_list = "".join(new_exemplar_list)
    ...
    if training_args.do_eval:
        ...
        valid_dataset = ICLDataset(valid_dataset, tokenizer, data_args.input_col_name,
                                   data_args.output_col_name, data_args.max_length,
                                   exemplar_list, is_eval=True)
    ...
    if training_args.do_predict:
        ...
        test_dataset = ICLDataset(test_dataset, tokenizer, data_args.input_col_name,
                                  data_args.output_col_name, data_args.max_length,
                                  exemplar_list, is_eval=True)
